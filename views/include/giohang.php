<?php
	if(isset($_POST['themgiohang'])){
		...
		$soluong = $_POST['soluong'];
		 ...
		 ///th tren la moi san pham chi lay 1sp
		 if($count >0){
		 	 ///th tren la moi san pham chi lay 2sp tro len
		...
		if($insert_row == 0){
	 	...
	 	//th khong them dc gio hang
		}
	}else if(isset($_POST['capnhatsoluong'])){
		
		
		//luc nay hai bien nay goi ra chuoi
	...
	// 	//huy viec dang nhap
		...
		$giaohang = $_POST['giaohang'];
		 ...
		 if($sql_khachhang){
		 	...
		 	//lay 1khach hang moi nhat
		 	$mahang = rand(0,9999);
		 	//cho ma hang random 4 so
		 	...
		 	for($i = 0;$i < count($_POST['thanhtoan_product_id']);$i++){
				...
				$sql_giaodich = mysqli_query($con,"INSERT INTO tbl_giaodich(sanpham_id,khachhang_id,giaodich_soluong,giaodich_ma) values('$sanpham_id','$khachhang_id','$giaodich_soluong','$mahang')");
				$sql_delete_thanhtoan = mysqli_query($con,"DELETE  FROM tbl_giohang  WHERE sanpham_id='$sanpham_id'");
				//them vao don hang thi gio hang se xoa cac san pham
	...
	<div class="container py-xl-4 py-lg-2">
		...
		<?php
			if(isset($_SESSION['dangnhap_home'])){
				echo '<h4 style="color:red;">Xin chào bạn: '.$_SESSION['dangnhap_home'].'.  <a href="index.php?quanly=giohang&dangxuat=1">Đăng xuất</a></h4><br/>';
			}else{
				echo '';
			}
		?>
						...
						<tbody>
							...
							<tr class="rem1">
								...
								<td class="invert-image">
									<a href="single.html">
										<img src="assets/images/<?php echo $row_fetch_giohang['hinhanh']?>" alt=" " class="img-responsive ">
									</a>
								</td>
								<td class="invert">
									<input type="hidden"   name="product_id[]" value="<?php  echo $row_fetch_giohang['sanpham_id'] ?>">
									<input type="number"   min="1"  max="10" name="soluong[]" value="<?php  echo $row_fetch_giohang['soluong'] ?>">
									<!--  [] để lấy nhiu sản phẩm -->
							...
							?>
							<tr>
								<td colspan="7">Tổng tiền: <?php echo number_format($toal). ' VNĐ' ?></td>
							...
							<tr>
								<td colspan="7"><input type="submit"  name="capnhatsoluong"  class="btn btn-success" value="Cập nhật giỏ hàng">
									...
									if(isset($_SESSION['dangnhap_home']) && $count_giohang_select > 0){
										while($row_laysp = mysqli_fetch_array($sql_giohang_select)){
									?>
									<input type="hidden"   name="thanhtoan_product_id[]" value="<?php  echo $row_laysp['sanpham_id'] ?>">
									<input type="hidden"   name="thanhtoan_soluong[]" value="<?php  echo $row_laysp['soluong'] ?>">
									<?php
										}
									?>
		...
		<div class="checkout-left">
			<div class="address_form_agile mt-sm-5 mt-4 container" >
							...
							<div class="first-row">
								...
								<div class="controls form-group">
									<select class="option-w3ls" name="giaohang">
										<option>Chọn hình thức toán</option>
